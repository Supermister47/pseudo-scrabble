...
// TEST AGREGADO
TEST_F(JuegosTests, jugada_valida_mas_grande) {
    Fachada_Variante v(tamanoTab, 3, puntajes, palabrasLegitimas);

    repo = {'a', 'e', 't', 'a', 'f', 'e', 't', 'g', 'w', 'r', 'e'};
    ...
    EXPECT_TRUE(juego.jugadaValida({{0, 0, 'a'}, {0, 2, 'e'}, {0, 1, 't'}}));
    juego.ubicar({{0, 0, 'a'}, {0, 2, 'e'}, {0, 1, 't'}});

    juego.mostrar(cout, true);
    EXPECT_EQ(juego.cantFicha(0, 'a'), 0);
    EXPECT_EQ(juego.cantFicha(0, 't'), 1);
    EXPECT_EQ(juego.cantFicha(1, 'f'), 1);
    EXPECT_EQ(juego.cantFicha(1, 'e'), 1);
